...
def createModel(modelType='LSTM', seqSize=1, featureSize=1):
    if (modelType == 'LSTM'):
        ...
        model.add(Conv1D(filters=20, kernel_size=3, activation='relu',
                         input_shape=(seqSize, featureSize)))
        model.add(Dropout(0.2))
        ...
        svm_poly = SVC(kernel='poly', C=100,
                       gamma='scale', degree=4, coef0=0.0)

        model = svm_poly
    elif (modelType == 'GDB'):
        model = GradientBoostingClassifier(
            loss='exponential', learning_rate=0.1, n_estimators=50)
...
def calcReturn(trainPredict, testPredict, data, ddata, look_back=20):
    ...
    rettsdata = data[len(data)-1, 0] / \
        data[len(traPredict)+(look_back*2)+1, 0] - 1
...
def showResult(sname, modelType, trainPredict, testPredict, trainY, testY, data, ddata, scaler, look_back=20):
    ...
    if (modelType == "SVM") or (modelType == "LSTMC") or (modelType == "CNN") or (modelType == "GDB"):
        ...
        traPredict = np.reshape(scaler.inverse_transform(
            np.reshape(traPredict, (len(traPredict), 1))), (len(traPredict)))
        traY = np.reshape(scaler.inverse_transform(
            np.reshape(traY, (len(traY), 1))), (len(traY)))
        tstPredict = np.reshape(scaler.inverse_transform(
            np.reshape(tstPredict, (len(tstPredict), 1))), (len(tstPredict)))
        tstY = np.reshape(scaler.inverse_transform(
            np.reshape(tstY, (len(tstY), 1))), (len(tstY)))
        ...
        tstPredictPlot[len(traPredict)+(look_back*2) +
                       1:len(data)-1] = tstPredict
    ...
    if (plotOnlyPredict):
        ...
        ax.set_xbound(lower=min(
            copy, key=lambda tup: tup[0])[0] - 10, upper=max(
            copy, key=lambda tup: tup[0])[0] + 10)
        ax.set_ybound(lower=min(
            copy, key=lambda tup: tup[1])[1] - 30, upper=max(
            copy, key=lambda tup: tup[1])[1] + 30)
...
for modelType in mtype:
    for sname in sym:
        ...
        trainX, trainY, testX, testY, data, ddata, scaler = create_sample(
            modelType, sname=sname, look_back=look_back, start=start)
        ...
        showResult(sname, modelType, trainPredict, testPredict,
                   trainY, testY, data, ddata, scaler, look_back=look_back)
# end
